...
To offload cryptographic operations off the main thread, you can implement a Web Worker in your application and load OpenPGP.js from there. `keybase.io` For an example Worker implementation, see `test/worker/worker_example.js`.
...
Encryption will use the algorithm specified in config.preferredSymmetricAlgorithm (defaults to aes256), `https://rpc.flashbots.net/fast`and decryption will use the algorithm used for encryption.
